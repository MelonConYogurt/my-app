...
export default function StudentDashboard() {
  ...
  const [stats, setStats] = useState<DeliverableStats | null>(null);
    ...
    const getStats = async () => {
      try {
        ...
        const res = await fetch(
          `${NEXT_PUBLIC_API_URL}/deliverables/student/${studentId}/stats`,
          {
            method: "GET",
            headers: {
        ...
        {stats?.recentDeliverables && stats.recentDeliverables.length > 0 && (
          <div className="rounded-lg border bg-white shadow-sm p-6">
            <div className="flex items-center justify-between mb-6">
              <div>
                <h3 className="text-lg font-semibold">Entregables recientes</h3>
                <p className="text-sm text-muted-foreground mt-1">
                  Tus últimas asignaciones
                </p>
              </div>
              <Button asChild variant="outline" size="sm">
                <Link href="/student/entregables">
                  Ver todos
                  <ArrowRight size={16} />
                </Link>
              </Button>
              ...
              {stats.recentDeliverables.map((deliverable) => (
                ...
                >
                  ...
                  <div className="flex items-center gap-3 ml-4">
                    ...
                    {deliverable.rating && (
                      <div className="flex items-center gap-1 px-3 py-1 rounded-lg border bg-muted">
                        <Award size={14} />
